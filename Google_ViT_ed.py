...
class ImageTransformer(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels=3, dropout=0.1, emb_dropout=0.1):
        ...
        self.pos_embedding = nn.Parameter(
            torch.empty(1, (num_patches + 1), dim))
        # initialized based on the paper
        torch.nn.init.normal_(self.pos_embedding, std=.02)
        # eqivalent to x matmul E, E= embedd matrix, this is the linear patch projection
        ...
        # if finetuning, just use a linear layer without further hidden layers (paper)
    ...
    def forward(self, img, mask=None):
        ...
        # each of 64 vecotrs is linearly transformed with a FFN equiv to E matmul
        ...
        #
        # print("img.shape[0] bsize", img.shape[0])
        #
...
# %%
# 藥牌
model = ImageTransformer(image_size=512, patch_size=16, num_classes=184, channels=3,
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    train(model, optimizer, ps1_ps2_train_loader, train_loss_history)

    print('Execution time:', '{:5.2f}'.format(
        time.time() - start_time), 'seconds')

    evaluate(model, ps1_ps2_test_loader, test_loss_history)
